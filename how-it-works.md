...
Lets start by checking the triggerred run using our CLI tool by running `hyperp list-runs` command, which is the only command that is available for interacting with the platform. We can use arrow keys to use the interactive tool and optionally pass `--commit-hash <commit hash>` parameter if we need to investigate a run not in the last 10 runs.

![Alt text](./docs/images/1_tyqoKE4cxXgN1MR6q21w7g.gif)
...
![Alt text](./docs/images/1_gRPNo6ejx8MLkwf4osYgGQ.gif)
...
![Alt text](./docs/images/1_dFBwf1D6XtIZy07xkZK_7g.png)
...
![Alt text](./docs/images/1_dH5NRZnfXrc_qQvmnDSPrg.png)
...
![Alt text](./docs/images/1_UPC6YOJGdrHkQwr55OOozg.png)
...
![Alt text](./docs/images/1_pSYoC5tWJG2lNSSvxv30LA.gif)
...
![Alt text](./docs/images/1_zM7b9g0eOEE8ZKyKewWm5w.gif)
...
Once the workflow finished, an [usage calculator](https://github.com/acikelli/hyperp/tree/main/utilities/usage-calculator) task is deployed to calculate the cummulitive CPU, Memory and storage usage. We can see price estimations of the usages on the CLI.
